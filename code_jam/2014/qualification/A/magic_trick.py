...
for i in range(no_test_case):
    ans = "Case #{}: ".format(i+1)
    select1 = int(in_file.readline().strip())
    cards1   = []
    for _ in range(4):
        cards1.append([int(x) for x in in_file.readline().strip().split()])
    select2 = int(in_file.readline().strip())
    cards2  = []
    for _ in range(4):
        cards2.append([int(x) for x in in_file.readline().strip().split()])
    #print(select1, select2, cards1, cards2)
    diffs = set(cards1[select1-1]).intersection(set(cards2[select2-1]))
    if len(diffs) == 1:
        out_file.write(ans +  str(next(iter(diffs))))
    elif len(diffs) > 1:
        out_file.write(ans + "Bad magician!")
    elif len(diffs) == 0:
        out_file.write(ans +"Volunteer cheated!")
    out_file.write("\n")
