...
https://leetcode.com/contest/leetcode-weekly-contest-54/problems/falling-squares/
699. Falling Squares
...
class Solution(object):
    def fallingSquares(self, positions):
        ...
        for start, size in positions:
            ...
            #pos = bisect.bisect_left(level, [newlv, []])
            pos = 0
            ...
            #print level, pos, "!"
            #new level
            if pos == len(level) or level[pos][0] != newlv:
                level.insert(pos, [newlv, [(start, end)]])
            else:
                level[pos].append((start, end))
            #print level, pos
            ret.append(level[0][0])
        return ret
