...
class Solution:
    def sampleStats(self, count):
        ...
        lens = len(count)
        result = [float(min(count)), float(max(count)), sum(count)/float(lens)]
        if lens % 2:median = float(count[lens//2])
        else: median = (count[lens//2] + count[lens//2 - 1]) / 2
        result.append(median)
        counter = Counter(count)
        max_count = max(count,key=count.count)
        result.append(float(max_count))
        return result
